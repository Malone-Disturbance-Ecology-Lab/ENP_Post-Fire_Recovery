...
# This script cleans and combines fire history shapefiles from Everglades National Park (EVER 1948-2020) and Big Cypress National Preserve (BICY 1978-2020)  
  # to create annual fire records for the whole Everglades landscape (EVG 1978-2020).
...
# Original shapefiels obtained from EVER are saved here:
    # "/Volumes/SERCDATA/Malone Lab/ENP Fire/Everglades Fire History/Spatial Layers/EVG FIRE PERIMETERS_original"
# Original shapefiels obtained from BICY are saved here:
...
# Some years have different columns and issues than others. These issues are resolved below
# 1948-2010 are formatted the same. Make following years match this format before applying additional edits to all years
# Edits only account for variables relevant to this study:
    # Fire_ID, Fire_Number, Fire_Name, DISC_DATE, DECLED_DATE, YEAR_, Fire_Type

# Some date edits were made manually in GIS prior to importing into R (see READ ME file). Saved here:
  # "/Volumes/inwedata/Malone Lab/ENP Fire/Grace_McLeod/Fire_History/EVER_FIRE_newdate"

  # Make a path to pull in the original files (post-GIS edits, not raw from the park) 
  original <- "/Volumes/inwedata/Malone Lab/ENP Fire/Grace_McLeod/Fire_History/EVER_fires_newdate"
  # Make a path to where geometrically cleaned files will be saved
  clean <- "/Volumes/inwedata/Malone Lab/ENP Fire/Grace_McLeod/Fire_History/EVER_fires_clean"
  # Make a path to where edited files will be saved
  GM <- "/Volumes/inwedata/Malone Lab/ENP Fire/Grace_McLeod/Fire_History/EVER_fire_edits_GM"
  
  
# IMPORT ALL SHAPEFILES (1948-2020) ................................................................................................................................
  # Clean polygon geometry and assign CRS
  
  setwd(original)
  All <-dir(original, "*.shp$")
  shp <- All[1:73]
  ...
  for (shp in shp){
    ...
    # Transform shapefile to the prefered crs
    sp <- spTransform(sp, crs("+proj=longlat +datum=WGS84 +no_defs")) 
    # Cleans the geometry of the shapefile
    sp <- clgeo_Clean(sp, errors.only = NULL, strategy = "POLYGONATION", verbose = FALSE) 
