...
# *** filter to just points that still need processing ***
# load processing pts
# Made in Sampling script
load("./Recovery/BonusPTS.RDATA")
process_df <- as.data.frame(process)
head(process_df)
# change ptID in Spec_Master
head(Spec_Master)
colnames(Spec_Master)[colnames(Spec_Master) == "ptID"] ="OGptID"
# keep only pts in Spec_Master also in process_df
Recov_Spec <- merge(Spec_Master, process_df,  by=c("coords.x1", "coords.x2")) # should be same number of pts as process_df
length(unique(Recov_Spec$ptID))
head(Recov_Spec)

# BAND MIN AND MAX CALUCULATIONS (only due bands necessary for baseline)
# Band Max per pt
#Recov_Spec$Pt.B1max<- NA
#Recov_Spec$Pt.B2max<- NA
#Recov_Spec$Pt.B3max<- NA
Recov_Spec$Pt.B4max<- NA
Recov_Spec$Pt.B5max<- NA
Recov_Spec$Pt.B7max<- NA
# Band min per pt
#Recov_Spec$Pt.B1min<- NA
#Recov_Spec$Pt.B2min<- NA
#Recov_Spec$Pt.B3min<- NA
Recov_Spec$Pt.B4min<- NA
#Recov_Spec$Pt.B5min<- NA
#Recov_Spec$Pt.B7min<- NA
...
for (i in unique(Recov_Spec$ptID)){
  ...
  # get min and max
  #Recov_Spec$Pt.B1min[Recov_Spec$ptID ==i] <-min(SubID$B1) #B1
  #Recov_Spec$Pt.B1max[Recov_Spec$ptID ==i] <-max(SubID$B1)
  #Recov_Spec$Pt.B2min[Recov_Spec$ptID ==i] <-min(SubID$B2) #B2
  #Recov_Spec$Pt.B2max[Recov_Spec$ptID ==i] <-max(SubID$B2)
  #Recov_Spec$Pt.B3min[Recov_Spec$ptID ==i] <-min(SubID$B3) #B3
  #Recov_Spec$Pt.B3max[Recov_Spec$ptID ==i] <-max(SubID$B3)
  Recov_Spec$Pt.B4min[Recov_Spec$ptID ==i] <-min(SubID$B4) #B4
  Recov_Spec$Pt.B4max[Recov_Spec$ptID ==i] <-max(SubID$B4)
  #Recov_Spec$Pt.B5min[Recov_Spec$ptID ==i] <-min(SubID$B5) #B5
  Recov_Spec$Pt.B5max[Recov_Spec$ptID ==i] <-max(SubID$B5)
  #Recov_Spec$Pt.B7min[Recov_Spec$ptID ==i] <-min(SubID$B7) #B7
...
# ***** merge with OG_Recov_Spec ******
# save bonus spec
Bonus_Spec <- Recov_Spec 
rm(Recov_Spec)
setwd("./Recovery")
save(Bonus_Spec, file="Bonus_Spec.RDATA")
# relaod bonus spec
load("./Bonus_Spec.RDATA")
# Use new_master for OG_pt spec
OG_Spec <- new_master %>% dplyr::select(ptID, coords.x1, coords.x2, Obs_Year, Obs_month, Obs_Date, 
                                     B1, B2, B3, B4,B5, B7, NDVI, NBR, NBR2, 
                                     Pt.B4max, Pt.B5max, Pt.B7max, Pt.B4min, NIR.SWIR1, SWIR1.SWIR2)
# match columns
Bonus_Spec <- Bonus_Spec %>% dplyr::select(ptID, coords.x1, coords.x2, Obs_Year, Obs_month, Obs_Date, 
                                           B1, B2, B3, B4,B5, B7, NDVI, NBR, NBR2, 
                                           Pt.B4max, Pt.B5max, Pt.B7max, Pt.B4min, NIR.SWIR1, SWIR1.SWIR2)
length(unique(OG_Spec$ptID)) # 61,771
length(unique(Bonus_Spec$ptID)) # 95,552
# merge 
class(Bonus_Spec$Obs_Year)
class(OG_Spec$Obs_Year)
Bonus_Spec$Obs_Year <- as.numeric(Bonus_Spec$Obs_Year)
Recov_Spec <- full_join(OG_Spec, Bonus_Spec, by=c( "ptID","coords.x1" ,"coords.x2" , "Obs_Year" , "Obs_month" , "Obs_Date" , 
                                                   "B1","B2","B3", "B4" , "B5" , "B7" , "NDVI",  "NBR" ,"NBR2" , 
                                                   "Pt.B4max","Pt.B5max"  ,  "Pt.B7max"  ,  "Pt.B4min"  ,  "NIR.SWIR1"  , "SWIR1.SWIR2")) # should be sum of obs of both
summary(Recov_Spec)
length(unique(Recov_Spec$ptID)) # 157,323
...
class(Recov_Spec$Obs_month)
class(Recov_DAYMET$Obs_month)
...
write.csv(Recov_Master, file="Recov_Master.csv")




# **** COMBINE WITH ALREADY PROCESSED RECOV_MASTER PTS *****
load("./Bonus_Master.RDATA")
load("./BonusPTS.RDATA")
process_df <- as.data.frame(process)
head(new_master)
head(Recov_Master)
...
names(Recov_Master)
# clean 
new_master <- new_master %>% dplyr::select(ptID, coords.x1, coords.x2, Obs_Year, Obs_month, Obs_Date, 
                                           B1, B2, B3, B4,B5, B7, NDVI, NBR, NBR2, 
                                           tmin, tmax, precip,
                                           FFYear, PenUltFY, TotalFires, FireName, FireNumber, StartDate, EndDate, FireYear, FireType, Prev.Int, TSF,
                                           Pt.B4max, Pt.B5max, Pt.B7max, Pt.B4min, NIR.SWIR1, SWIR1.SWIR2)
Recov_Master <- Recov_Master %>% dplyr::select(ptID, coords.x1, coords.x2, Obs_Year, Obs_month, Obs_Date, 
                                               B1, B2, B3, B4,B5, B7, NDVI, NBR, NBR2, 
                                               tmin, tmax, precip,
                                               FFYear, PenUltFY, TotalFires, FireName, FireNumber, StartDate, EndDate, FireYear, FireType, Prev.Int, TSF,
                                               Pt.B4max, Pt.B5max, Pt.B7max, Pt.B4min, NIR.SWIR1, SWIR1.SWIR2)
# merge
length(unique(new_master$ptID)) # 61,771
length(unique(Recov_Master$ptID)) # 95,552
Recov_Master <- full_join(new_master, Recov_Master, by=c( "ptID","coords.x1" ,"coords.x2" , "Obs_Year" , "Obs_month" , "Obs_Date" , 
                                                          "B1","B2","B3", "B4" , "B5" , "B7" , "NDVI",  "NBR" ,"NBR2" , 
                                                          "tmin" ,"tmax" , "precip",
                                                          "FFYear" ,"PenUltFY" ,"TotalFires" , "FireName"   , "FireNumber" , "StartDate"  , "EndDate"  , "FireYear" ,"FireType" , "Prev.Int" ,"TSF",  
                                                          "Pt.B4max","Pt.B5max"  ,  "Pt.B7max"  ,  "Pt.B4min"  ,  "NIR.SWIR1"  , "SWIR1.SWIR2")) # should be sum of obs of both
10824730+17701462 # = 28526192
length(unique(Recov_Master$ptID)) # 157,323
head(Recov_Master)
# save 
write.csv(Recov_Master, file="Recov_Master.csv")
save(Recov_Master, file="Recov_Master.RDATA")
